...
rm -rf /usr/share/kct-nimp-agent
...
wget --inet4-only https://raw.githubusercontent.com/OLSSOO-Inc/DIPCAST-CUSTOM/master/kct/nimp-agent/KCT_Agent_ver2.2b.zip
mkdir -p /usr/share/kct-nimp-agent
unzip KCT_Agent_ver2.2b.zip -d  /usr/share/
mv  /usr/share/KCT_Agent_ver2.2b /usr/share/kct-nimp-agent
chmod +x  /usr/share/kct-nimp-agent/agent.sh

echo -e "${green}Configuring KCT NIMP AGENT v2.2b...${txtrst}"
...
ExecStart=/root/.sdkman/candidates/java/current/bin/java -jar amagent_22b.jar NIMP_AGENT_22b
...
echo -e "${green}Starting & Enabling KCT-Nimp-Agent Services..v2.2b..${txtrst}"
